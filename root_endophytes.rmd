...
pca_factors <- data.table(
  ...
  site_var = sapply(pcas, function(x) (x['trial', 'Sum.Sq'] / sum(x['Sum.Sq'])) * 100),
  site_p = sapply(pcas, function(x) x['trial', 'Pr..F.']),
...
correlate <- function(df, canker_count, measures, corr_methods, return) {
  mat <- sapply(
    corr_methods, 
    function(x) sapply(
      measures, 
