...
pcs_factors_tidy <- lapply(
  ...
  function(name) {
    mutate(
      ...
      # total_var = pca_var[name, "perc_var"]
    ) %>%
    mutate(
      pcas[[name]],
      total_var = var * pca_var[name, "perc_var"] / 100
    )
  }
...
# Significant factors
kable(pcs_factors_tidy[
  Pr..F. < 0.05, 
  c("PC", "Factor", "Df", "F.value", "Pr..F.", "var", "total_var")
])
```
...
```{r}
# Export significant OTUS as fasta
scion_otus <- as.character(otu_anova_results_adjusted[scion_p < 0.05, ]$OTU)
FUN_otus <- read.fasta("data/FUN.otus.fa")
write.fasta(
  sequences = FUN_otus[scion_otus],
  names = paste(scion_otus, "ITS"),
  file = "FUN_scion_otus.fasta"
)
```
...
asv_canker_anova <- function(otu, design, base_model, data) {
  ...
  d = data.frame(
    ...
    Taxonomy = taxData[otu, "rank"],
    Abundance = mean(data[[otu]]),
    coef = m$fit$coefficients[log_otu],
...
# Search for significant ASVs the `FUN.otus.fa` fasta file and write to a new file
FUN_otus <- read.fasta("data/FUN.otus.fa")
write.fasta(
  sequences = FUN_otus[as.character(significant_asvs$OTU)],
  names = paste(significant_asvs$OTU, "ITS"),
  file = "FUN_canker_otus.fasta"
)
```
...
significant_asv_list <- significant_asvs$OTU %>% unlist()
...
significant_asv_long_data <- significant_asv_data %>% reshape2::melt(
  id.vars = c("Site", "Storage", "Scion", "Cankers"), variable.name = "OTU", value.name = "Abundance"
)
...
fun_asv_canker_plot <- ggscatter(
  ...
  color = "Storage", facet.by = c("OTU", "Site"),
...
# Export significant OTUS as fasta
scion_otus <- as.character(otu_anova_results_adjusted[scion_p < 0.05, ]$OTU)
BAC_otus <- read.fasta("data/BAC.otus.fa")
write.fasta(
  sequences = BAC_otus[scion_otus],
  names = paste(scion_otus, "16S"),
  file = "BAC_scion_otus.fasta"
)
```
...
BAC_otus <- read.fasta("data/BAC.otus.fa")
write.fasta(
  sequences = BAC_otus[as.character(significant_asvs$OTU)],
  names = paste(significant_asvs$OTU, "16S"),
  file = "BAC_canker_otus.fasta"
)
```
