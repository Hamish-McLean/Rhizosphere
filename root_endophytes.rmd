...
# ANOVA of ASV abundance with canker count per site
...
# Significant ASVs
significant_asvs <- lapply(
  asv_canker_site_results, 
  function(x) x[p_adjusted < 0.05 & is.na(warning), ]
)

kable(significant_asvs)
```
...
significant_asv_list <- lapply(significant_asvs, function(x) x$OTU) %>% unlist()
