...
fun_genus_data <- countData %>% mutate(
...
# Rank not genus
not_genus <- fun_genus_data$genus[grep("\\([a-z]\\)", fun_genus_data$genus)]
# Remove rows with genus in not_genus
fun_genus_data <- fun_genus_data[!fun_genus_data$genus %in% not_genus, ]
cat(
...
fun_genera <- fun_genus_data$genus
rownames(fun_genus_data) <- fun_genera
...
```{r}
# Add genus from taxData to countData
bac_genus_data <- countData %>% mutate(
  genus = taxData[rownames(countData), "genus"]
)
...
# Rank not genus
not_genus <- bac_genus_data$genus[grep("\\([a-z]\\)", bac_genus_data$genus)]
# Remove rows with genus in not_genus
bac_genus_data <- bac_genus_data[!bac_genus_data$genus %in% not_genus, ]
cat(
  length(not_genus), "non-genus ranks removed:\n\n",
  not_genus, "\n"
)
...
removals <- bac_genus_data$genus[grep("[/_]", bac_genus_data$genus)]
# Remove rows with symbols in the name
bac_genus_data <- bac_genus_data[!bac_genus_data$genus %in% removals, ]
cat(
...
# Set rownames as genus
bac_genera <- bac_genus_data$genus
rownames(bac_genus_data) <- bac_genera
