...
```{r fun canker model}
canker_model <- glm.nb(canker_design, data = cankers)
```
...
# Canker model with ASV abundance data
canker_asv_model <- glm.nb(canker_design, data = top_asv_data)

# ANOVA of top ASVs with canker count
asv_canker_anova <- function(otu, data) {
  log_otu = paste0("log(", otu, " + 1)")
  f = paste(canker_design, "+", log_otu)
  m = glm.nb(f, data = data)
  a = anova(canker_asv_model, m) %>% data.frame()
...
# kable(asv_canker_anova(top_asv_ids[1], top_asv_data))
...
alpha_canker_anova <- function(measure, data) {
  ...
  m = glm.nb(f, data = data)
  a = anova(canker_model, m) %>% data.frame()
  b = anova(m) %>% data.frame()
  total_deviance = sum(b$Deviance, na.rm = T) + tail(b$Resid..Dev, 1)
  d = data.frame(
    measure = measure,
