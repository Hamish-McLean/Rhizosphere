...
```{r FUN ASV ANOVA, eval = ASV_FACTOR}
# ANOVA of top ASVs
...
```{r FUN ASV ANOVA with canker, eval = ASV_CANKER}
# ANOVA of top ASVs
...
```{r FUN ASV canker, eval = CANKER_ASV}
# Filter out samples with missing canker count
...
```{r FUN ASV canker per site, eval = CANKER_ASV}
...
```{r FUN ASV canker FASTA, eval = FALSE}
# Export significant ASVs as FASTA
write.fasta(
...
```{r BAC ASV ANOVA, eval = ASV_FACTOR}
# Perform ANOVA on list of top ASVs
top_asvs_anova_results <- lapply(
  top_asv_ids, 
  function(asv) {
...
```{r BAC ASV ANOVA with canker, eval = ASV_CANKER}
...
```{r BAC ASV canker, eval = CANKER_ASV}
# Filter out samples with missing canker count
...
```{r BAC ASV canker per site, eval = CANKER_ASV}
...
## α-diversity per site

```{r α-diversity per site figures}
shannon_canker_site_plot <- ggscatter(
  data = bac_alpha, y = "shannon", x = "Cankers", facet.by = "Site",  
  scales = "free_y",
  add = "reg.line", conf.int = T, cor.coef = T
)

ggsave(
  filename = "shannon_site_plot.png", plot = shannon_canker_site_plot, path = "figures/", 
  height = 20, width = 24, units = "cm"
)

simpson_canker_site_plot <- ggscatter(
  data = bac_alpha, y = "simpson", x = "Cankers", facet.by = "Site",  
  scales = "free_y",
  add = "reg.line", conf.int = T, cor.coef = T
)

ggsave(
  filename = "simpson_site_plot.png", plot = simpson_canker_site_plot, path = "figures/", 
  height = 20, width = 24, units = "cm"
)


alpha_canker_site_plot <- ggarrange(
  shannon_canker_site_plot, simpson_canker_site_plot,
  ncol = 1, nrow = 2, labels = c("A", "B"),
  common.legend = T, legend = "bottom"
)

ggsave(
  filename = "alpha_canker_site_plot.png", plot = alpha_canker_site_plot, path = "figures/", 
  height = 20, width = 24, units = "cm"
)
```
...
```{r abundance α-diversity figures}
box_palette <- c("Winter" = "#8da0cb", "Spring" = "#fc8d62")

abundance_plot <- ggboxplot(
  data = all_combined, x = "Site", y = "TCN", 
  color = "Season", add = "jitter", facet.by = "kingdom",
  palette = box_palette, scales = "free", legend = "bottom",
  ylab = "TCN (log10)", xlab = "Site"
) # + labs(colour = "Planting season")
chao1_plot <- ggboxplot(
  data = all_combined, x = "Site", y = "Chao1", 
  color = "Season", add = "jitter", facet.by = "kingdom",
  palette = box_palette, scales = "free_y", legend = "bottom",
...
ggsave(
  ...
  height = 24, width = 18, units = "cm", dpi = 300, bg = "white"
)
...
```{r abundance α-diversity facet figures}
abundance_alpha_facet <- ggboxplot(
  data = alpha_combined, x = "Site", y = "value", 
  color = "Storage", add = "jitter", facet.by = c("measure", "kingdom"),
  palette = cbPalette, scales = "free_y", legend = "bottom"
  # ylab = "Mean diversity index", xlab = "Site", 
  # labeller = c(TCN = "TCN (log10)", Chao1 = "Chao1 index", Shannon = "Shannon index", Simpson = "Simpson index")
) #+ guides(color = guide_legend(position = "right"))
ggsave(
...
nmds_combo_plot <- ggarrange(
  ...
  ncol = 2, nrow = 1, labels = c("A", "B"), widths = c(1.19, 1),
  common.legend = T, legend = "bottom"
)

ggsave(
  filename = "nmds_combo_plot.png", plot = nmds_combo_plot, path = "figures/", 
  height = 13, width = 24, units = "cm", dpi = 300, bg = "white"
)
