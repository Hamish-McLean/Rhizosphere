...
ubiome_BAC$colData <- ubiome_BAC$colData %>% rename(Site = site, Scion = cultivar)
ubiome_FUN$colData <- ubiome_FUN$colData %>% rename(Site = site, Scion = cultivar)
...
cankers_bar <- ggbarplot(
  cankers, x = "Site", y = "total_cankers", fill = "cultivar", 
...
fun_abundance_bar <- ggbarplot(
  data = as.data.frame(colData(dds)), x = "planting_season", y = "log_copy_number", 
...
pca_factors <- data.table(
  ...
  site_cultivar_var = sapply(pcas, function(x) (x['site:cultivar', 'Sum.Sq'] / sum(x['Sum.Sq']) * 100)),
  site_cultivar_p = sapply(pcas, function(x) x['site:cultivar', 'Pr..F.']),
  site_season_var = sapply(pcas, function(x) (x['site:planting_season ', 'Sum.Sq'] / sum(x['Sum.Sq']) * 100)),
  site_season_p = sapply(pcas, function(x) x['site:planting_season ', 'Pr..F.']),
...
otu_lm_anova <- function(otu, formula, data) {
  ...
  d = data.frame(
    ...
    site_p = a['site ', 'Pr..F.'],
    cultivar_var = a['cultivar', 'Sum.Sq'] / total_var * 100,
    cultivar_p = a['cultivar', 'Pr..F.'],
    season_var = a['planting_season ', 'Sum.Sq'] / total_var * 100,
    season_p = a['planting_season ', 'Pr..F.'],
    site_cultivar_var = a['site:cultivar', 'Sum.Sq'] / total_var * 100,
    site_cultivar_p = a['site:cultivar', 'Pr..F.'],
    site_season_var = a['site:planting_season ', 'Sum.Sq'] / total_var * 100,
    site_season_p = a['site:planting_season ', 'Pr..F.'],
...
# otu_lm_anova(top_otu_ids[1], formula, top_otu_data)

otu_anova_results <- sapply(
  top_otu_ids, function(x) otu_lm_anova(x, formula, top_otu_data)
) %>% t() %>% data.table()
...
cat(
  ...
  "Cultivar:", nrow(otu_anova_results_adjusted[cultivar_p < 0.05, ]), "\n",
  "Season:", nrow(otu_anova_results_adjusted[season_p < 0.05, ]), "\n",
  "Site:Cultivar:", nrow(otu_anova_results_adjusted[site_cultivar_p < 0.05, ]), "\n",
...
# asv_canker_anova(top_otu_ids[1], top_otu_data)
...
abundance_bar <- ggbarplot(
  data = abundance_combined, x = "planting_season", y = "log_copy_number", 
...
alpha_bar <- ggbarplot(
  data = alpha_combined, x = "planting_season", y = "value", 
