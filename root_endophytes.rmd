...
```{r}
# Remove sample with id "A2-7" from colData and countData
...
all_canker_data <- all_canker_data %>% 
  semi_join(ubiome_FUN$colData, by = c("id", "planting_season"))
...
all_canker_data$mainstem <- all_canker_data$a4 + all_canker_data$b4
all_canker_data$peripheral <- all_canker_data$c4 + all_canker_data$d4 + all_canker_data$e4
...
canker_data <- all_canker_data %>%
  ...
  summarise(
    mainstem = mean(mainstem, na.rm = T),
    peripheral = mean(peripheral, na.rm = T)
  )

# Add canker data to colData for both FUN and BAC
ubiome_FUN$colData <- merge(
  rownames_to_column(ubiome_FUN$colData, var = "Row.names"), 
  canker_data[, c("id", "mainstem", "peripheral")], 
  by = "id"
...
```{r}
canker_data <- fread("canker_data.csv", select = 1:34)

# Clean up canker data



# Sum mainstem and peripheral canker counts
canker_data$M1 <- canker_data$A1 + canker_data$B1
canker_data$P1 <- canker_data$C1 + canker_data$D1 + canker_data$E1

canker_data$M2 <- canker_data$A2 + canker_data$B2
canker_data$P2 <- canker_data$C2 + canker_data$D2 + canker_data$E2

canker_data$M3 <- canker_data$A3 + canker_data$B3
canker_data$P3 <- canker_data$C3 + canker_data$D3 + canker_data$E3

canker_data$M4 <- canker_data$A4 + canker_data$B4
canker_data$P4 <- canker_data$C4 + canker_data$D4 + canker_data$E4

# Calculate area under the disease progression curve
audpc <- function(counts, timepoints) {
  # Calculate average count between timepoints
  avg <- (counts[-1] + counts[-length(counts)]) / 2

  # Calculate time between timepoints
  #time <- timepoints[-1] - timepoints[-length(timepoints)]
  time <- diff(timepoints)
...
dates <- c("2019-10-08", "2020-06-01", "2020-11-11", "2021-05-01")
timepoints <- as.Date(dates, format = "%Y-%m-%d")

canker_data$mainstem_auc <- apply(
  canker_data[, c("M1", "M2", "M3", "M4")], 
  1, audpc, timepoints = timepoints
)

canker_data$peripheral_auc <- apply(
  canker_data[, c("P1", "P2", "P3", "P4")], 
  1, audpc, timepoints = timepoints
)
