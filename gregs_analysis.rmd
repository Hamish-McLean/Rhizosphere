...
ubiome_zBAC <- loadData("data/BAC.zotu_table.txt",metadata,"data/zBAC.sintax.taxa",RHB="BAC")
ubiome_zFUN <- loadData("data/FUN.zotu_table.txt",metadata,"data/zFUN.sintax.taxa",RHB="FUN")
```
...
ubiome_zFUN$dds2 <- ubiom_to_des(ubiome_zFUN,filter=expression(colSums(countData)>=1000))
ubiome_zBAC$dds2 <- ubiom_to_des(ubiome_zBAC,filter=expression(colSums(countData)>=1000))
...
g1 <- gfunc(as.data.frame(counts(dds2)),as.data.frame(colData(dds2)),"Bacteria")
invisible(mapply(assign,names(ubiome_FUN),ubiome_FUN,MoreArgs=list(envir = globalenv())))
g2 <- gfunc(as.data.frame(counts(dds2)),as.data.frame(colData(dds2)),"Fungi")

# changed all occurences of dds to dds2
...
plot_alpha(counts(dds2,normalize=T),
           colData(dds2),
           design=Factor1,
           colour=Factor2,
