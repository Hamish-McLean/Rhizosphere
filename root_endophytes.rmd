...
asv_canker_anova <- function(otu, data) {
  ...
  f = paste(canker_design, "+", log_otu)#, "+", log_otu, ":Site")
  m = glm.nb(f, data = data)
...
# kable(asv_canker_anova(top_asv_ids[1], canker_top_asv_data))

# Effect of ASV abundance on canker count for top ASVs
asv_canker_results <- data.table(t(sapply(top_asv_ids, function(x) asv_canker_anova(x, canker_top_asv_data))))
