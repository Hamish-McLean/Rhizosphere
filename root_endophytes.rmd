...
cankers_hist <- ggplot(cankers, aes(x = total_cankers)) + 
  geom_histogram(bins = 20) + 
  facet_grid(site ~ .) +
  labs(title = "Total canker count", x = "Total canker count")

cankers_hist

ggsave(filename = "cankers_hist.png", plot = cankers_hist, path = "figures/")

cankers_scatter <- ggplot(cankers, aes(x = site, y = total_cankers, colour = cultivar)) + 
  geom_jitter() +
  labs(title = "Total canker count", x = "Site", y = "Total canker count")

cankers_scatter

ggsave(filename = "cankers_scatter.png", plot = cankers_scatter, path = "figures/")

cankers_box <- ggplot(cankers, aes(x = site, y = total_cankers, fill = planting_season)) + 
  geom_boxplot() +
  labs(title = "Total canker count", x = "Site", y = "Total canker count")
...
# Log-linear model
canker_model <- lm(update(FULL_DESIGN, log(total_cankers + 1) ~ .), data = cankers)

# Residual checking
res <- resid(canker_model, type = "pearson")

plot(fitted(canker_model), res) 
abline(0,0, col="red")
...
# Poisson model
canker_model <- glm(update(FULL_DESIGN, total_cankers ~ .), data = cankers, family = "poisson")

plot(simulateResiduals(canker_model))

# Model overdispersed

# Negative binomial model
canker_model <- glm.nb(update(FULL_DESIGN, total_cankers ~ .), data = cankers)

sim <- simulateResiduals(canker_model)
plot(sim)
...
Anova(canker_model, type = "II", test = "LR")
...
asv_canker_anova <- function(otu, data) {
  ...
  m = Anova(glm(f, data = data, family = "poisson"), type = "II", test = "F")
  a = data.frame(m)
  d = data.frame(
    OTU = otu,
    Taxonomy = taxData[otu, "rank"],
    var = a[otu, 'Sum.Sq'] / sum(a$Sum.Sq) * 100,
    p = a[otu, 'Pr..F.']
  )
  return(d)
}
...
alpha_canker_anova <- function(measure, data) {
  ...
  d = data.frame(
    measure = measure,
    var = a[measure, 'Sum.Sq'] / sum(a$Sum.Sq) * 100,
    p = a[measure, 'Pr..F.']
  )
  return(d)
}
...
beta_canker_anova <- function(pc, data) {
  ...
  d = data.frame(
    PC = pc,
    var = a[pc, 'Sum.Sq'] / sum(a$Sum.Sq) * 100,
    p = a[pc, 'Pr..F.']
  )
  return(d)
}
...
# Formula for ANOVA
formula <- update(DESIGN, log_copy_number ~ .)
