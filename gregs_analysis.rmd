...
normaliseCountData <- function(data) {
  ...
  totalOTUs <- as.data.frame(totalOTUs, col.names = c("ID", "TotalOTUs"))
  
  # adjustedTotalOTUs <- totalOTUs %>%
  #   map(~ .x * data$abundance[data$abundance$ID == .x, "MeanAdjustedTCN"])
  return(totalOTUs)
}

str(normaliseCountData(ubiome_BAC))
