...
md1[, Season := factor(Season, levels = c("Spring", "Winter"))]
...
md2[, Season := factor(Season, levels = c("Spring", "Winter"))]
...
alpha_combined <- rbind(fun_alpha, bac_alpha) %>% 
  subset(select = c("Site", "Season", "Scion", "Target", "S.chao1", "shannon", "simpson")) %>%
  mutate(kingdom = ifelse(Target == "ITS", "Fungi", "Bacteria")) %>%
  mutate(kingdom = factor(kingdom, levels = c("Fungi", "Bacteria"))) %>%
  mutate(Season = factor(Season, levels = c("Winter", "Spring"))) %>%
...
alpha_box_fungi <- ggboxplot(
  ...
  color = "Season", add = "jitter", facet.by = "measure",
  palette = cbPalette, scales = "free_y", legend = "bottom",
  ylab = "Mean diversity index", xlab = "Site"
)
ggsave(
