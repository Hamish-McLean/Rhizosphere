...
names(ubiome_BAC$taxData) <- gsub("OTU", "ASV", names(ubiome_BAC$taxData))
names(ubiome_FUN$taxData) <- gsub("OTU", "ASV", names(ubiome_FUN$taxData))

names(ubiome_BAC$countData) <- gsub("OTU", "ASV", names(ubiome_BAC$countData))
names(ubiome_FUN$countData) <- gsub("OTU", "ASV", names(ubiome_FUN$countData))
```
...
# asv_lm_anova(top_asv_ids[1], formula, top_asv_data)

asv_anova_results <- sapply(
  top_asv_ids, function(x) asv_lm_anova(x, formula, top_asv_data)
) %>% t() %>% data.table()

asv_anova_results_adjusted <- asv_anova_results %>% 
  mutate(
...
cat(
  ...
  "Site:", nrow(asv_anova_results_adjusted[site_p < 0.05, ]), "\n",
  "Scion:", nrow(asv_anova_results_adjusted[scion_p < 0.05, ]), "\n",
  "Storage:", nrow(asv_anova_results_adjusted[storage_p < 0.05, ]), "\n",
  "Site:Scion:", nrow(asv_anova_results_adjusted[site_scion_p < 0.05, ]), "\n",
  "Site:Storage:", nrow(asv_anova_results_adjusted[site_storage_p < 0.05, ]), "\n",
  "Storage:Scion:", nrow(asv_anova_results_adjusted[storage_scion_p < 0.05, ]), "\n\n"
)

asv_anova_results_adjusted[scion_p < 0.05, ]
asv_anova_results_adjusted[site_scion_p < 0.05, ]
asv_anova_results_adjusted[storage_scion_p < 0.05, ]
```

```{r}
# Export significant ASVs as fasta

# Filter ASVs with significant Scion effect
scion_asvs <- as.character(asv_anova_results_adjusted[scion_p < 0.05, ]$ASV)
...
write.fasta(
  sequences = FUN_asvs[scion_asvs],
  names = paste(scion_asvs, taxData[scion_asvs, "rank"]),
  file = "FUN_scion_asvs.fasta"
)
```
...
# Write significant ASVs to fasta
write.fasta(
  sequences = BAC_asvs[scion_asvs],
  names = paste(scion_asvs, taxData[scion_asvs, "rank"]),
  file = "BAC_scion_asvs.fasta"
)
```
