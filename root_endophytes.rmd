...
### α-diversity ~ canker permutation anova
...
canker_formula <- y ~ Site * Storage * Scion * Cankers

# Chao1

setkey(all_alpha_ord, S.chao1)
all_alpha_ord[, measure := as.numeric(as.factor(S.chao1))]
result <- aovp(update(canker_formula, measure ~ .), all_alpha_ord, seqs = T)
summary(result)
df <- result %>% summary() %>% unclass() %>% data.frame()
total_variance <- sum(df$R.Sum.Sq)
df$Perc.Var <- df$R.Sum.Sq / total_variance * 100
